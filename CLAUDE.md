...
- **Multicursor** = `Vec<Range>`; one edit applies to all (post-M0).
...
- **Non-goal:** Zed-tier rendering/perf (Actually this is a goal but not for the prototype version), collaboration, huge-file engineering, terminal/TTY support, building for hypothetical contributors (stay modular so the door stays open at zero cost).
...
| Coordinate model | store byte offsets; move by grapheme clusters; derive visual columns (M0 moves by `char`) |
| Undo/redo | transaction/changeset + revision-tree history, anchor-based (post-M0) |
| Syntax highlighting | tree-sitter (post-M0) |
| Config / keymap file | declarative, serialized `Keymap` (post-M0) |
...
2. Plan tasks needing live confirmation are flagged **[verify against pinned rev]** — actually verify them against your checkout.
